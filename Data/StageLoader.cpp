...
bool FStageLoader::IsLoaded() const
{
    return bLoaded;
}
...
bool FStageLoader::LoadStageById(int StageIndex, FStageData &OutStage) const
{
    ...
    try
    {
        ...
        {
            if (!Meta["intro_messages"].is_array())
            {
                return false;
            }

            for (const auto &MessageJson : Meta["intro_messages"])
            {
                ...
                OutStage.AddIntroMessage(MessageJson.get<std::string>());
            }
        }
        ...
        if (Meta.contains("items") && Meta["items"].is_array())
        {
            for (const auto &ItemJson : Meta["items"])
            {
                ...
                const std::string TypeStr = ItemJson.value("type", std::string{});
