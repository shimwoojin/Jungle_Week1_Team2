...
void FActor::Update(float DeltaTime)
{
	if (bIsMoving)
	{
		MoveTimer += DeltaTime;
...
bool FActor::TryMove(FStage& Stage, EDirection Dir)
{
	...
	switch (Dir)
	{
	case EDirection::Up:    NextY--; break;
	case EDirection::Down:  NextY++; break;
	case EDirection::Left:  NextX--; break;
	case EDirection::Right: NextX++; break;
	}
...
int FActor::GetTileX() const
{
	return TileX;
}

int FActor::GetTileY() const
{
	return TileY;
}

float FActor::GetRenderX() const
{
	return RenderX;
}

float FActor::GetRenderY() const
{
	return RenderY;
}

void FActor::SetDirection(EDirection InDirection)
{
	Direction = InDirection;
}

EDirection FActor::GetDirection() const
{
	return Direction;
}

void FActor::SetHp(int InHp)
{
	Hp = InHp;
}

int FActor::GetHp() const
{
	return Hp;
}
...
void FActor::Heal(int Amount)
{
	Hp += Amount;
}

bool FActor::IsDead() const
{
	return Hp <= 0;
}

bool FActor::IsMoving() const
{
	return bIsMoving;
}
