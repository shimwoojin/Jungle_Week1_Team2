...
void FRenderer::PrepareShader() { BindShader(EShaderType::Default); }
...
void FRenderer::DrawSprite(const FTexture *Texture, float WorldX, float WorldY, float Width,
                           float Height, const FSpriteInfo &Sprite,
                           const DirectX::XMFLOAT4 &ColorTint)
{
    ...
    XMMATRIX World =
        XMMatrixScaling(Width, Height, 1.0f) * XMMatrixTranslation(WorldX, WorldY, 0.0f);
...
void FRenderer::DrawTexture(const FTexture *texture, float screenX, float screenY, float width,
                            float height, const DirectX::XMFLOAT4 &ColorTint)
{
    FRenderObject Obj;
    Obj.Texture = texture;

    XMMATRIX World =
        XMMatrixScaling(width, height, 1.0f) * XMMatrixTranslation(screenX, screenY, 0.0f);
    XMStoreFloat4x4(&Obj.World, XMMatrixTranspose(World));
...
void FRenderer::DrawTexture(const FTexture *texture, float screenX, float screenY, float width,
                            float height, float rotationRad, const DirectX::XMFLOAT4 &ColorTint)
{
    FRenderObject Obj;
    Obj.Texture = texture;

    XMMATRIX World = XMMatrixScaling(width, height, 1.0f) * XMMatrixRotationZ(rotationRad) *
...
void FRenderer::DrawBatch(const FStaticBatch &Batch, const FTexture *Texture)
{
    ...
    int Idx = static_cast<int>(BatchRenderCmds.size());
    BatchRenderCmds.push_back({&Batch, Texture});
    RenderCommands.push_back({ERenderCmdType::Batch, Idx});
}

void FRenderer::Render()
{
    if (RenderCommands.empty() && FontRenderObjects.empty())
        return;

    // 스크린 스페이스용 View/Projection (캐시)
    XMFLOAT4X4 ScreenView, ScreenProjection;
    XMStoreFloat4x4(&ScreenView, XMMatrixTranspose(XMMatrixIdentity()));
    XMStoreFloat4x4(&ScreenProjection,
                    XMMatrixTranspose(XMMatrixOrthographicOffCenterLH(
                        0.0f, (float)ScreenWidth, (float)ScreenHeight, 0.0f, 0.0f, 1.0f)));

    // 통합 커맨드 큐 순회
    for (const FRenderCommand &Cmd : RenderCommands)
    {
        switch (Cmd.Type)
        {
        case ERenderCmdType::Batch:
            ExecuteBatch(BatchRenderCmds[Cmd.Index]);
            break;
        case ERenderCmdType::Sprite:
            ExecuteSprite(RenderObjects[Cmd.Index]);
            break;
        case ERenderCmdType::DarknessOverlay:
            ExecuteDarkness(DarknessRenderCmds[Cmd.Index]);
            break;
        }
    }

    // Font rendering (UI shader) — 항상 최후에 렌더
    if (!FontRenderObjects.empty())
    {
        ...
        for (const FFontRenderObject &RenderObject : FontRenderObjects)
        {
            ...
            FVertexSimple textVertices[6] = {
                {-0.5f, -0.5f, 0.f, UV0.X, UV0.Y}, {0.5f, -0.5f, 0.f, UV1.X, UV0.Y},
                {0.5f, 0.5f, 0.f, UV1.X, UV1.Y},

                {-0.5f, -0.5f, 0.f, UV0.X, UV0.Y}, {0.5f, 0.5f, 0.f, UV1.X, UV1.Y},
                {-0.5f, 0.5f, 0.f, UV0.X, UV1.Y},
            };
...
// ============================================================
// Execute helpers (통합 렌더 큐에서 호출)
// ============================================================

void FRenderer::ExecuteBatch(const FBatchRenderCmd &Cmd)
{
    PrepareShader();

    UINT stride = Stride;
    UINT offset = 0;
    DeviceContext->IASetVertexBuffers(0, 1, &Cmd.pBatch->VertexBuffer, &stride, &offset);
    DeviceContext->IASetIndexBuffer(Cmd.pBatch->IndexBuffer, DXGI_FORMAT_R32_UINT, 0);
    DeviceContext->PSSetSamplers(0, 1, &SamplerState);

    if (Cmd.Texture && Cmd.Texture->GetTextureSRV())
    {
        ID3D11ShaderResourceView *SRV = Cmd.Texture->GetTextureSRV();
        DeviceContext->PSSetShaderResources(0, 1, &SRV);
    }
    ...
    if (Cmd.Texture)
    {
        XMFLOAT2 TexSize = {(float)Cmd.Texture->Width, (float)Cmd.Texture->Height};
    ...
    DeviceContext->DrawIndexed(Cmd.pBatch->IndexCount, 0, 0);
}

void FRenderer::ExecuteSprite(const FRenderObject &Obj)
{
    PrepareShader();
...
void FRenderer::ExecuteDarkness(const FDarknessRenderCmd &Cmd)
{
    ...
    XMMATRIX         World = XMMatrixScaling(QW, QH, 1.0f) *
                     XMMatrixTranslation(Cmd.ScreenCenterX, Cmd.ScreenCenterY, 0.0f);
    XMStoreFloat4x4(&CB.World, XMMatrixTranspose(World));
    XMStoreFloat4x4(&CB.View, XMMatrixTranspose(XMMatrixIdentity()));
    XMStoreFloat4x4(&CB.Projection, XMMatrixTranspose(XMMatrixOrthographicOffCenterLH(
                                        0.0f, W, H, 0.0f, 0.0f, 1.0f)));
    XMFLOAT2 TexSize = {(float)Cmd.Texture->Width, (float)Cmd.Texture->Height};
    CB.SpriteSize = TexSize;
    CB.TextureSize = TexSize;
...
bool FRenderer::LoadShaderFromFile(const std::wstring &Path)
{
    ...
    HRESULT Hr = D3DCompileFromFile(Path.c_str(), nullptr, nullptr, "mainVS", "vs_5_0", 0, 0,
                                    &VsBlob, &ErrorBlob);
    if (FAILED(Hr))
    {
        if (ErrorBlob)
        {
            ShaderError = "VS: " + std::string((const char *)ErrorBlob->GetBufferPointer(),
    ...
    Hr = D3DCompileFromFile(Path.c_str(), nullptr, nullptr, "mainPS", "ps_5_0", 0, 0, &PsBlob,
                            &ErrorBlob);
    if (FAILED(Hr))
    {
        if (ErrorBlob)
        {
            ShaderError = "PS: " + std::string((const char *)ErrorBlob->GetBufferPointer(),
                                               ErrorBlob->GetBufferSize());
            ErrorBlob->Release();
        }
        else
...
const std::string &FRenderer::GetShaderError() const { return ShaderError; }
...
const std::string &FRenderer::GetCurrentShaderName() const { return CurrentShaderName; }
...
void FRenderer::CreateShader()
{
    ...
    D3DCompileFromFile(L"Resources/Shaders/Default.hlsl", nullptr, nullptr, "mainVS", "vs_5_0", 0,
    ...
    D3DCompileFromFile(L"Resources/Shaders/Default.hlsl", nullptr, nullptr, "mainPS", "ps_5_0", 0,
...
void FRenderer::CreateFontShader()
{
    ...
    D3DCompileFromFile(L"Resources/Shaders/UI.hlsl", nullptr, nullptr, "mainVS", "vs_5_0", 0, 0,
                       &VsBlob, nullptr);
    Device->CreateVertexShader(VsBlob->GetBufferPointer(), VsBlob->GetBufferSize(), nullptr,
                               &FontVertexShader);

    D3DCompileFromFile(L"Resources/Shaders/UI.hlsl", nullptr, nullptr, "mainPS", "ps_5_0", 0, 0,
                       &PsBlob, nullptr);
    ...
    Device->CreateInputLayout(layout, ARRAYSIZE(layout), VsBlob->GetBufferPointer(),
                              VsBlob->GetBufferSize(), &FontInputLayout);
...
void FRenderer::SwapBuffer() { SwapChain->Present(1, 0); }
...
void FRenderer::DrawDarknessOverlay(const FTexture *Texture, float ScreenCenterX,
                                    float ScreenCenterY)
{
    if (!Texture || !Texture->GetTextureSRV())
        return;

    int Idx = static_cast<int>(DarknessRenderCmds.size());
    DarknessRenderCmds.push_back({Texture, ScreenCenterX, ScreenCenterY});
    RenderCommands.push_back({ERenderCmdType::DarknessOverlay, Idx});
}
...
void FRenderer::CreateSimpleQuad()
{
    ...
    Device->CreateBuffer(&bufferDesc, &initData, &QuadBuffer);
}
...
void FRenderer::ReleaseVertexBuffer(ID3D11Buffer *vertexBuffer) { vertexBuffer->Release(); }
...
bool FRenderer::Initialize(HWND hWindow, int InScreenWidth, int InScreenHeight)
{
    ...
    XMStoreFloat4x4(&CachedProjection,
                    XMMatrixTranspose(XMMatrixOrthographicOffCenterLH(
                        0.0f, (float)ScreenWidth, (float)ScreenHeight, 0.0f, 0.0f, 1.0f)));

    RenderObjects.reserve(256);
    RenderCommands.reserve(256);
    BatchRenderCmds.reserve(16);
    DarknessRenderCmds.reserve(4);

    return Device != nullptr;
}

void FRenderer::BeginFrame() { Prepare(); }
...
void FRenderer::EndFrame()
{
    ...
    RenderObjects.clear();
    BatchRenderCmds.clear();
    DarknessRenderCmds.clear();
    FontRenderObjects.clear();
}

int FRenderer::GetScreenWidth() const { return static_cast<int>(ViewportInfo.Width); }

int FRenderer::GetScreenHeight() const { return static_cast<int>(ViewportInfo.Height); }
